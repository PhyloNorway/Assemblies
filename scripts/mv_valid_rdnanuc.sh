...
#DORGANNOT=dorgannot
CHECK="${SCRIPT_DIR}/check_rDNA.sh"
...
simpler=${fasta_file/.rdnanuc.raw.fasta/}
library=${simpler##*.}
...
if [[ -z $specimen ]]; then
    ...
    output_file="$output_dir/${scientific_name}.${specimen_id}.${library}.rdnanuc.raw.fasta.gz"
 ...
 #   && rm -rf "$TMPDIR" 
    ...
    if  ${CHECK} "../data/embl/${embl_file}" > "../data/md/$markdown_file" ; then
        obiconvert $1 | gzip > "$output_file" \
        || { rm -f "$output_file"; echo "Failed to convert $embl_file to fasta format."; exit 1;} \
    else
        echo "Invalid rDNA" 1>&2
        touch "$output_dir/${invalid_file}"
        exit 1
    fi
fi
